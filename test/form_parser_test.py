...
def test_parses_strings_without_coercion():
	...
	assert value_parser.parse("title") == "title"
...
def test_parses_uuid():
	...
	assert value_parser.parse(raw) == UUID(raw)
...
def test_parses_optional_blank_and_value():
	...
	assert value_parser.parse("") is None
	assert value_parser.parse(raw) == UUID(raw)
...
def test_parses_checked_boolean_strictly():
	...
	assert value_parser.parse("on") is True
...
def test_parses_list_from_scalar_and_list():
	...
	assert value_parser.parse(first) == [UUID(first)]
	assert value_parser.parse([first, second]) == [UUID(first), UUID(second)]
